...
pub struct PathTracerBase<'a> {
    ...
    pub prev_roughness_interval: Var<f32>,
    ...
    pub debug_shift_mapping_pixels: Option<[Expr<Uint2>; 2]>,
}
...
pub struct BsdfDenoisingFeatures {
    ...
    pub roughness_interval: Expr<f32>,
}
impl<'a> PathTracerBase<'a> {
    pub fn new(
        ...
        Self {
            ...
            prev_roughness_interval: Var::<f32>::zeroed(),
            swl,
            color_pipeline,
            force_diffuse: false,
            ...
            debug_shift_mapping_pixels: None,
        }
    }
    #[tracked(crate = "luisa")]
    ...
    pub fn sample_surface_and_shade_direct(
        ...
        let sample_and_shade = |closure: &SurfaceClosure| {
            ...
            let roughness_interval = sm
                .map(|sm| closure.roughness_interval(wo, sm.min_roughness, **self.swl, &ctx))
                .unwrap_or(0.0f32.expr());
            (
                sample,
        ...
        loop {
                    ...
                    if !sm_vertex.valid() & sm.is_base_path & is_last_vertex {
                        sm.write_vertex(ReconnectionVertex::from_comps_expr(
                            ReconnectionVertexComps {
                                ...
                                prev_roughness_interval: **self.prev_roughness_interval,
                                roughness_interval: 0.0f32.expr(),
                                u_bsdf_select: -1.0f32.expr(),
                                dist,
                                depth: **self.depth,
                                type_: VertexType::LAST_HIT_LIGHT.expr(),
                                // prev_roughness: **self.prev_roughness,
                        ...
                        if self.enable_debug {
                            device_log!(
                                "rejected last_vertex, tid:{}, depth:{}, prev_roughness:{}",
                                dispatch_id(),
                                self.depth,
                                self.prev_roughness
                            );
            ...
            *self.depth += 1;
            ...
            let (bsdf_sample, features, direct) = if always_reconnect_first_hit {
                ...
                self.sample_surface_and_shade_direct(
                    None,
                    si,
                    wo,
                    direct_lighting,
            ...
            let roughness_interval = features.roughness_interval;
            let roughness_criteria = shift_mapping.map(|sm| roughness >= sm.min_roughness);
            // let roughness_interval = shift_mapping.map(|sm| {
            //     self.dispatch_surface(None, si.surface, si, **self.swl, &|closure| {
            //         closure.roughness_interval(
            //             wo,
            //             sm.min_roughness,
            //             **self.swl,
            //             &self.eval_context().1,
            ...
            if let Some(sm) = &shift_mapping {
                ...
                if !sm.is_base_path & sm_vertex.valid() {
                    if self.depth > self.min_reconnect_depth {
                        ...
                        if can_connect {
                            // if self.enable_debug {
                            //     if let Some(from_to) = self.debug_shift_mapping_pixels {
                            //         let v = sm.read_vertex();
                            //         // if v.valid() {
                            //         let from = from_to[0];
                            //         let to = from_to[1];
                            //         device_log!(
                            //         "tid:{}, early reject! {} -> {}, depth: {},prev_roughness:{} roughness:{}, vertex:{}",
                            //         dispatch_id(),
                            //         from,
                            //         to,
                            //         self.depth,
                    ...
                    if sm_vertex.depth == self.depth {
                                ...
                                let e = self.dispatch_surface(
                                    ...
                                    **self.swl,
                                    &|closure| {
                                        ...
                                        //     &self.eval_context().1,
                                        // );
                                        // let interval = closure.roughness_interval(
                                        //     wo,
                                        //     sm.min_roughness,
                                        //     **self.swl,
                                        //     &self.eval_context().1,
                        ...
                        // commect this makes stuff working?
                        // if roughness < sm.min_roughness {
                        //     if self.enable_debug {
                        //         if let Some(from_to) = self.debug_shift_mapping_pixels {
                        //             let from = from_to[0];
                        //             let to = from_to[1];
                        //             device_log!(
                        //             "reject roughness 1 bad. tid: {} {} -> {}, roughness_y1: {}, prev_roughness: {},depth:{} vertex:{}",
                        //             dispatch_id(),
                        //             from,
                        //             to,
                        //             roughness,
                        ...
                        let (direct_f, roughness_y2, f2, pdf_y2, interval_y2) = {
                            ...
                            outline(|| {
                                let e = self.dispatch_surface(
                                    ...
                                    **self.swl,
                                    &|closure| {
                                        let (roughness_y, f2, pdf_y2, interval_y2) =
                                        ...
                                        (direct_f, roughness_y, f2, pdf_y2, interval_y2)
                                    },
                                );
                            ...
                            & (roughness_y2 < sm.min_roughness)
                        {
                            ...
                            //         device_log!(
                            //     "reject roughness 2 bad. tid: {} {} -> {}, roughness_y1: {}, roughness_y2: {},depth:{} vertex:{}",
                            //     dispatch_id(),
                            //     from,
                            //     to,
                            //     roughness,
                        ...
                        let roughness_interval_ratio = compute_ratio(
                            1.0 - roughness_interval,
                            1.0 - reconnection_vertex.prev_roughness_interval,
                        )
                        .var();
                        let pdf_ratio = (pdf_y1 / reconnection_vertex.prev_bsdf_pdf).var();
                        if vertex_type != VertexType::LAST_HIT_LIGHT {
                            *pdf_ratio *= compute_ratio(pdf_y2, reconnection_vertex.bsdf_pdf);
                            *roughness_interval_ratio *= compute_ratio(
                                1.0 - interval_y2,
                                1.0 - reconnection_vertex.roughness_interval,
                            );
                        }
                        if pdf_ratio <= 0.0 {
                            *rejected = true;
                        ...
                        let jacobian = pdf_ratio / roughness_interval_ratio
            ...
            if always_reconnect_first_hit | rejected {
                break;
            }
            ...
            if let Some(sm) = &shift_mapping {
                if self.depth > self.min_reconnect_depth {
                    ...
                    //     if !sm.is_base_path {
                    //         device_log!(
                    //             "tid:{} d:{} dist: {}, roughness: {}, {},can_connect :{}",
                    //             dispatch_id(),
                    //             **self.depth,
                    //             dist,
                    //             roughness,
                    ...
                    if !reconnect_vertex.valid() & sm.is_base_path & can_connect {
                        sm.write_vertex(ReconnectionVertex::from_comps_expr(
                            ReconnectionVertexComps {
                                ...
                                // roughness: roughness,
                                prev_roughness_interval: **self.prev_roughness_interval,
                                roughness_interval: roughness_interval,
                            },
                        ));
                        self.reconnect_beta
                    ...
                    if !sm.is_base_path & can_connect {
                        ...
                        //         let to = from_to[1];
                        //         device_log!(
                        //             "tid:{}, early! {} -> {}, depth: {}, roughness:{}vertex:{}",
                        //             dispatch_id(),
                        //             from,
                        //             to,
                        //             self.depth,
            ...
            }
            {
                ...
                *self.prev_roughness_interval = roughness_interval;
...
pub struct ReconnectionVertex {
    ...
    pub bsdf_pdf: f32,
    pub prev_roughness_interval: f32,
    pub roughness_interval: f32,
    pub u_bsdf_select: f32,
    pub dist: f32,
    pub depth: u32,
    pub type_: u32,
    // for debug
    // pub prev_roughness: f32,
    // pub roughness: f32,
}
