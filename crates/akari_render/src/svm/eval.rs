...
#[tracked(crate = "luisa")]
fn read_data_slice_at<T: Value>(
    shader_data: &BufferVar<u8>,
    shader_ref: Expr<ShaderRef>,
    cst: SvmConst<T>,
    idx: Expr<u32>,
) -> Expr<T> {
    shader_data
        .read_as::<T>(cst.offset + shader_ref.data_offset + idx * std::mem::size_of::<T>() as u32)
}
impl<'a> SvmEvaluator<'a> {
    ...
    fn do_eval(&mut self, node: SvmNodeRef) -> Evaluated {
        ...
        match node {
            ...
            svm::SvmNode::Toon(toon) => {
                let surface = self.eval_bsdf_closure(toon.surface);
                let color_band_width = self.eval_float(toon.color_band_width);
                wrap_any!(Rc::new(ToonSurface {
                    inner: surface.clone(),
                    color_band_width
                }) as Rc<dyn Surface>)
            }
            svm::SvmNode::EightBit(eb) => {
                let surface = self.eval_bsdf_closure(eb.surface);
                let scale = self.eval_float(eb.indirect_scale);
                wrap_any!(Rc::new(EightBitSurface {
                    inner: surface.clone(),
                    indirect_scale: scale
                }) as Rc<dyn Surface>)
            }
            svm::SvmNode::HalfTone(ht) => {
                let surface = self.eval_bsdf_closure(ht.surface);
                let indirect_scale = self.eval_float(ht.indirect_scale);
                let radius_scale = self.eval_float(ht.radius_scale);
                let radius = self.eval_float(ht.radius);
                let t: Expr<AffineTransform> = self.eval(ht.affine);
                let d = self.eval_float(ht.grid_distance);
                let max_depth = self.eval(ht.max_depth);
                let light_tone = self.eval_color(ht.light_tone);
                let dark_tone = self.eval_color(ht.dark_tone);
                let blend = self.eval_float(ht.blend);
                wrap_any!(Rc::new(HalfToneSurface {
                    inner: surface.clone(),
                    indirect_scale,
                    affine: t,
                    radius,
            ...
            svm::SvmNode::SaturationAdjust(sa) => {
                let surface = self.eval_bsdf_closure(sa.surface);
                let power = self.eval_float(sa.power);
                let indirect_scale = self.eval_float(sa.indirect_scale);
                let max_depth = self.eval(sa.max_depth);
                wrap_any!(Rc::new(SaturationAdjustSurface {
                    inner: surface.clone(),
                    power,
                    indirect_scale,
                    max_depth
            ...
            svm::SvmNode::SaturationAdjustRex(sar) => {
                let surface = self.eval_bsdf_closure(sar.surface);
                let blend = self.eval_float(sar.blend);
                wrap_any!(Rc::new(SaturationAdjustRexSurface {
                    inner: surface.clone(),
                    blend
                }) as Rc<dyn Surface>)
            }
            svm::SvmNode::ColorPalette(cp) => {
                let surface = self.eval_bsdf_closure(cp.surface);
                let indirect_scale = self.eval_float(cp.indirect_scale);
                let palette_len = self.read_data(cp.length);
                let shader_data = self.shader_data.clone();
                let shader_ref = self.shader_ref;
                let palette = cp.palette;
                let gamma = self.eval_float(cp.gamma);
                let surface = Rc::new(ColorPaletteSurface {
                    inner: surface.clone(),
                    indirect_scale,
                    palette_len,
                    palette_read_fn: Rc::new(move |idx: Expr<u32>| {
                        read_data_slice_at(&shader_data, shader_ref, palette, idx)
                    }),
                    gamma,
                }) as Rc<dyn Surface>;
                wrap_any!(surface)
            }
            svm::SvmNode::SetStyleIdx(ssi) => {
                let surface = self.eval_bsdf_closure(ssi.surface);
                let index = self.read_data(ssi.idx);
                wrap_any!(Rc::new(EnableOrSetStylizationSurface {
                    inner: surface.clone(),
                    index,
                    mode_set: true,
                    else_: Rc::new(NullSurface {})
                }) as Rc<dyn Surface>)
            }
            svm::SvmNode::EnableStylizationIf(esif) => {
                let surface = self.eval_bsdf_closure(esif.surface);
                let index = self.read_data(esif.idx);
                let else_ = self.eval_bsdf_closure(esif.else_);
                wrap_any!(Rc::new(EnableOrSetStylizationSurface {
                    inner: surface.clone(),
                    index,
                    mode_set: false,
            ...
            svm::SvmNode::Null(_) => {
                wrap_any!(Rc::new(NullSurface {}) as Rc<dyn Surface>)
            }
        }
    }
