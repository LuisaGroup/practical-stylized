...
use color::{
    f32_linear_to_srgb1, hsv_to_rgb, linear_to_srgb, linear_to_srgb1, rgb_to_hsv, srgb_to_linear,
    srgb_to_linear1,
};
...
#[luisa(crate = "luisa")]
#[repr(C)]
/// Additional flags for stylization
/// if stylization_disabled set to true, subsequent bounces will have  no stylization
/// the rest of the flags is a one-hot encoding representing which style to use.
///  by default, all of them are set to false, which means the first style is used.
pub struct StylizationFlags {
    pub stylization_disabled: bool,
    pub use_style_2: bool,
    pub use_style_3: bool,
    pub use_style_4: bool,
}
impl StylizationFlagsExpr {
    #[tracked(crate = "luisa")]
    pub fn get_style_index(&self) -> Expr<u32> {
        let index = 0u32.var();
        if self.use_style_2 {
            *index = 1;
        }
    ...
    #[tracked(crate = "luisa")]
    pub fn is_equal(&self, other: Expr<StylizationFlags>) -> Expr<bool> {
        (self.stylization_disabled == other.stylization_disabled)
            & (self.use_style_2 == other.use_style_2)
            & (self.use_style_3 == other.use_style_3)
            & (self.use_style_4 == other.use_style_4)
    }
}
impl StylizationFlagsVar {
    #[tracked(crate = "luisa")]
    pub fn set_style_index(&self, index: Expr<u32>) {
        *self.use_style_2 = index == 1;
        *self.use_style_3 = index == 2;
        *self.use_style_4 = index == 3;
    }
}
...
pub trait Surface {
    ...
    /// lobe roughness interval
    fn roughness_interval_impl(
        &self,
        wo: Expr<Float3>,
        roughness_threshold: Expr<f32>,
        swl: Expr<SampledWavelengths>,
        ctx: &BsdfEvalContext,
    ) -> Expr<f32>;
    ...
    fn avg_roughness(
        ...
        // lc_assert!(u_select.ge(0.0));
        let ret = Var::<f32>::zeroed();
        maybe_outline(|| ret.store(self.avg_roughness_impl(wo, swl, ctx)));
        ret.load()
        // (u_select > 0.8).select(1.0f32.expr(), 0.0f32.expr())
        // u_select
    }
    #[tracked(crate = "luisa")]
    fn roughness(
    ...
    fn roughness_interval(
        ...
        let ret = 0.0f32.var();
        maybe_outline(|| {
            *ret = self.roughness_interval_impl(wo, roughness_threshold, swl, ctx);
        });
        **ret
    }
    fn emission(
    ...
    fn is_stylized(&self, _path_depth: Expr<u32>, _flags: Expr<StylizationFlags>) -> Expr<bool> {
        false.expr()
    }
    /// called after each bounce
    fn update_stylization_flags(
        &self,
        _path_depth: Expr<u32>,
        _si: SurfaceInteraction,
        _wo: Expr<Float3>,
        flags: Expr<StylizationFlags>,
    ) -> Expr<StylizationFlags> {
        flags
    }
    fn apply_stylization(
        &self,
        _path_depth: Expr<u32>,
        _si: SurfaceInteraction,
        _wo: Expr<Float3>,
...
impl Surface for TransparentSurface {
    ...
    fn avg_roughness_impl(
        ...
        self.inner.avg_roughness(wo, swl, ctx)
    }
    #[tracked(crate = "luisa")]
    fn roughness_interval_impl(
        &self,
        ...
        self.alpha
            * self
                .inner
                .roughness_interval(wo, roughness_threshold, swl, ctx)
    }
    #[tracked(crate = "luisa")]
    fn sample_wi_impl(
        &self,
...
pub struct ToonSurface {
    pub inner: Rc<dyn Surface>,
    pub color_band_width: Expr<f32>,
}
impl Surface for ToonSurface {
    fn alpha(&self) -> Expr<f32> {
        self.inner.alpha()
    }
    fn ns(&self) -> Expr<Float3> {
    ...
    fn emission_impl(
        ...
        self.inner.emission(wo, swl, ctx)
    }
    fn is_stylized(&self, _path_depth: Expr<u32>, _flags: Expr<StylizationFlags>) -> Expr<bool> {
        true.expr()
    }
    ...
    fn apply_stylization(
        ...
        let li = self
            .inner
            .apply_stylization(path_depth, si, wo, li, ctx, flags);
        // todo!();
        // device_log!("lum: {}", lum);

        // if lum > self.color_band_width {
        //     albedo
        // } else {
        //     albedo * 0.1f32.expr()
        // }
        let li = li.clamp(100000.0f32.expr());
        let lum = li.lum();
        if lum <= 0.0 {
            li
        } else {
            let hue = li / lum;
            if lum > 1.0 {
                let new_lum = (lum / self.color_band_width).floor() * self.color_band_width;
                hue * new_lum
            } else {
                let adjusted_lum = linear_to_srgb1(lum);
                let new_lum =
                    (adjusted_lum / self.color_band_width).floor() * self.color_band_width;
                let new_lum = srgb_to_linear1(new_lum);
                hue * new_lum
            }
        }
    }
}
pub struct ColorPaletteSurface {
    pub inner: Rc<dyn Surface>,
    pub indirect_scale: Expr<f32>,
    pub palette_len: Expr<u32>,
    pub palette_read_fn: Rc<dyn Fn(Expr<u32>) -> Expr<Float3>>,
    pub gamma: Expr<f32>,
}
impl Surface for ColorPaletteSurface {
    fn alpha(&self) -> Expr<f32> {
        self.inner.alpha()
    }
    fn ns(&self) -> Expr<Float3> {
    ...
    #[tracked(crate = "luisa")]
    fn is_stylized(&self, _: Expr<u32>, _flags: Expr<StylizationFlags>) -> Expr<bool> {
        true.expr()
    }
    #[tracked(crate = "luisa")]
    fn apply_stylization(
        ...
        let li = self
            .inner
            .apply_stylization(path_depth, si, wo, li, ctx, flags);
        let rgb = li.as_rgb().var();
        let lum = rgb.reduce_sum() / 3.0;
        if path_depth != 0 {
            *rgb /= self.indirect_scale;
        }
        *rgb = rgb.clamp(Float3::splat_expr(0.0), Float3::splat_expr(1.0));
        *rgb = rgb.powf(self.gamma);
        let min_dist = 1e16f32.var();
        let min_dist_color = Float3::var_zeroed();
        for i in 0u32.expr()..self.palette_len {
            let c = (self.palette_read_fn)(i);
            // device_log!("{}", c);
            let dist = (c - **rgb).length_squared();
            if dist < **min_dist {
                *min_dist = dist;
                *min_dist_color = c;
            }
        }
        *rgb = min_dist_color.powf(1.0 / self.gamma);
        if path_depth != 0 {
            let new_lum = rgb.reduce_sum() / 3.0;
            if new_lum != 0.0 {
                *rgb *= lum / new_lum;
            }
        }
        Color::from_flat(ctx.color_repr, rgb.extend(0.0))
    }
}
pub struct SaturationAdjustSurface {
    pub inner: Rc<dyn Surface>,
    pub power: Expr<f32>,
    pub indirect_scale: Expr<f32>,
    pub max_depth: Expr<u32>,
}
impl Surface for SaturationAdjustSurface {
    fn alpha(&self) -> Expr<f32> {
        self.inner.alpha()
    }
    fn ns(&self) -> Expr<Float3> {
    ...
    #[tracked(crate = "luisa")]
    fn is_stylized(&self, path_depth: Expr<u32>, flags: Expr<StylizationFlags>) -> Expr<bool> {
        path_depth <= self.max_depth || self.inner.is_stylized(path_depth, flags)
    }
    #[tracked(crate = "luisa")]
    fn apply_stylization(
        &self,
        ...
        if lum == 0.0 {
            ...
            *rgb = linear_to_srgb(**rgb);
            let hsv = rgb_to_hsv(**rgb).var();
            *hsv.y = hsv.y.powf(self.power);
            *rgb = srgb_to_linear(hsv_to_rgb(**hsv));
            if path_depth != 0 {
                *rgb *= self.indirect_scale;
            }
            Color::from_flat(ctx.color_repr, rgb.extend(0.0))
...
pub struct EnableOrSetStylizationSurface {
    pub inner: Rc<dyn Surface>,
    pub mode_set: bool,
    pub index: Expr<u32>,
    pub else_: Rc<dyn Surface>,
}
impl Surface for EnableOrSetStylizationSurface {
    fn alpha(&self) -> Expr<f32> {
        self.inner.alpha()
    }
    fn ns(&self) -> Expr<Float3> {
    ...
    fn is_stylized(&self, path_depth: Expr<u32>, flags: Expr<StylizationFlags>) -> Expr<bool> {
        if self.mode_set {
            self.inner.is_stylized(path_depth, flags)
        } else {
            flags.get_style_index() == self.index
        }
    }
    #[tracked(crate = "luisa")]
    fn update_stylization_flags(
        ...
        let flags = flags.var();
        if self.mode_set {
            flags.set_style_index(self.index);
        }
        **flags
    }
    #[tracked(crate = "luisa")]
    fn apply_stylization(
        ...
        if self.mode_set {
            ...
            if flags.get_style_index() == self.index {
                self.inner
                    .apply_stylization(path_depth, si, wo, li, ctx, flags)
            } else {
                self.else_
...
pub struct SaturationAdjustRexSurface {
    pub inner: Rc<dyn Surface>,
    pub blend: Expr<f32>,
}
impl Surface for SaturationAdjustRexSurface {
    fn alpha(&self) -> Expr<f32> {
        self.inner.alpha()
    }
    fn ns(&self) -> Expr<Float3> {
    ...
    fn apply_stylization(
        ...
        let li = self
            ...
            .apply_stylization(path_depth, si, wo, li, ctx, flags);
        let rgb = li.as_rgb().var();
        let lum = (rgb.x + rgb.y + rgb.z) / 3.0;
        if lum <= 0.0 || rgb.reduce_min() < 0.0 {
            li
        } else {
            let c_min = rgb.reduce_min();
            let c_max = rgb.reduce_max();
            let i_min = Float3::splat_expr(lum);
            let i_max = (rgb - c_min) / c_max;
            let u = (i_max.x + i_max.y + i_max.z) / 3.0;
            if u != 0.0 {
                *rgb = self.blend * i_min + (1.0 - self.blend) * i_max * lum / u;
            } else {
                *rgb = self.blend * i_min + (1.0 - self.blend) * Float3::splat_expr(1.0) * lum;
            }
            Color::from_flat(ctx.color_repr, rgb.extend(0.0))
        }
    }
}
pub struct HalfToneSurface {
    pub inner: Rc<dyn Surface>,
    pub indirect_scale: Expr<f32>,
    pub radius_scale: Expr<f32>,
    pub affine: Expr<AffineTransform>,
    pub radius: Expr<f32>,
    pub d: Expr<f32>,
    pub max_depth: Expr<u32>,
    pub blend: Expr<f32>,
    pub dark_tone: Color,
    pub light_tone: Color,
}
impl Surface for HalfToneSurface {
    fn alpha(&self) -> Expr<f32> {
        self.inner.alpha()
    }
    fn ns(&self) -> Expr<Float3> {
    ...
    fn apply_stylization(
        ...
        if path_depth > self.max_depth {
            ...
            if lum == 0.0 {
                ...
                let r = if path_depth == 0 {
                    lum / self.radius_scale
                } else {
                    (lum / self.indirect_scale).round() / self.radius_scale
                };
                let r = 1.0 - r.clamp(0.0f32.expr(), 1.0f32.expr());
                let r = r * self.d * 0.5 * self.radius;
                let p = si.p;
                let ip = (self.affine.transform_point(p) / self.d).floor() * self.d;
                // find nearst grid point
                let nearest_gp = Float3::var_zeroed();
                let min_dist = 1e16f32.var();
                for_unrolled(0u32..2u32, |i| {
                    for_unrolled(0u32..2u32, |j| {
                        for_unrolled(0u32..2u32, |k| {
                            let q = ip + Float3::expr(i as f32, j as f32, k as f32) * self.d;
                            let dist = (q - p).length();
                            if dist < min_dist {
                                *min_dist = dist;
                                *nearest_gp = q;
                ...
                li.lerp_f32(c, self.blend)
            }
        }
    }
}
pub struct EightBitSurface {
    pub inner: Rc<dyn Surface>,
    pub indirect_scale: Expr<f32>,
}
impl Surface for EightBitSurface {
    fn alpha(&self) -> Expr<f32> {
        self.inner.alpha()
    }
    fn ns(&self) -> Expr<Float3> {
    ...
    fn apply_stylization(
        ...
        let map_8bit = |rgb: Expr<Float3>| -> Expr<Float3> {
            let rgb = linear_to_srgb(rgb);
            let r = (rgb.x * 8.0).round() / 8.0;
            let g = (rgb.y * 8.0).round() / 8.0;
            let b = (rgb.z * 4.0).round() / 4.0;
            srgb_to_linear(Float3::expr(r, g, b))
        };
        let lum = li.max();
        if lum == 0.0 {
            li
        } else {
            let rgb = if path_depth > 0 {
                map_8bit(li.as_rgb() / self.indirect_scale) * self.indirect_scale
            } else {
                let new_lum = lum.min_(1.0);
                let li = li / lum * new_lum;
                map_8bit(li.as_rgb())
            };
            Color::from_flat(ctx.color_repr, rgb.extend(0.0))
        }
...
impl Surface for EmissiveSurface {
    ...
    fn avg_roughness_impl(
        ...
        if let Some(inner) = &self.inner {
            inner.avg_roughness(wo, swl, ctx)
        } else {
            1.0f32.expr()
        }
    }
    fn roughness_interval_impl(
        ...
        if let Some(inner) = &self.inner {
            inner.roughness_interval(wo, roughness_threshold, swl, ctx)
        } else {
            0.0f32.expr()
        }
    }
...
impl Surface for BsdfMixture {
    ...
    fn avg_roughness_impl(
        ...
        let frac: Expr<f32> = (self.frac)(wo, ctx);
        let a = self.bsdf_a.avg_roughness(wo, swl, ctx);
        let b = self.bsdf_b.avg_roughness(wo, swl, ctx);
        a.lerp(b, frac)
    }
    #[tracked(crate = "luisa")]
    fn roughness_interval_impl(
...
impl Surface for SurfaceClosure {
    ...
    fn avg_roughness_impl(
        ...
        self.inner.avg_roughness(self.frame.to_local(wo), swl, ctx)
    }
    fn roughness_interval_impl(
        &self,
        wo: Expr<Float3>,
        ...
        self.inner
            .roughness_interval(self.frame.to_local(wo), roughness_threshold, swl, ctx)
    }
    fn emission_impl(
        &self,
        wo: Expr<Float3>,
    ...
    fn update_stylization_flags(
        ...
        self.inner
            .update_stylization_flags(path_depth, si, wo, flags)
    }
    #[tracked(crate = "luisa")]
    fn is_stylized(&self, path_depth: Expr<u32>, flags: Expr<StylizationFlags>) -> Expr<bool> {
    ...
    fn apply_stylization(
        ...
        if flags.stylization_disabled {
            ...
            // device_log!("apply_stylization {} {} {} {} {} {}", path_depth, si.p, si.inst_id, si.prim_id, si.bary, li.as_rgb());
            lc_assert!(si.p.length().gt(0.0));
            self.inner
                .apply_stylization(path_depth, si, self.frame.to_local(wo), li, ctx, flags)
        }
    }
}
impl SurfaceClosure {
...
impl Surface for MicrofacetReflection {
    ...
    fn roughness_interval_impl(
        ...
        let r = self.roughness_impl(wo, 0.0.expr(), swl, ctx);
        (r < roughness_threshold).select(1.0f32.expr(), 0.0f32.expr())
    }
    fn emission_impl(
        &self,
        _wo: Expr<Float3>,
