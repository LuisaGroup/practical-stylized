use color::SampledWavelengths;

use super::diffuse::DiffuseBsdf;
use super::{fr_dielectric_integral, BsdfEvalContext, SurfaceShader};
use std::rc::Rc;
...
impl Surface for PlasticBsdf {
    ...
    fn avg_roughness_impl(
        ...
        let fo = fr_dielectric(Frame::abs_cos_theta(wo), eta);
        let substrate_weight = substrate_weight(fo, self.kd_weight);
        let s = self.substrate.avg_roughness(wo, swl, ctx);
        let c = self.coat.avg_roughness(wo, swl, ctx);
        substrate_weight * s + (1.0 - substrate_weight) * c
    }
    #[tracked(crate = "luisa")]
    fn roughness_interval_impl(
        ...
        let fo = fr_dielectric(Frame::abs_cos_theta(wo), eta);
        let substrate_weight = substrate_weight(fo, self.kd_weight);
        let i_s = self.substrate.roughness_interval(wo, roughness_threshold, swl, ctx);
        let i_c = self.coat.roughness_interval(wo, roughness_threshold, swl, ctx);
        substrate_weight * i_s + (1.0 - substrate_weight) * i_c
    }
    fn emission_impl(
        &self,
        _wo: Expr<Float3>,
