...
#[tracked(crate = "luisa")]
pub fn srgb_to_linear1(rgb: Expr<f32>) -> Expr<f32> {
    rgb.le(0.04045)
        .select(rgb / 12.92, ((rgb + 0.055) / 1.055).powf(2.4))
}
#[tracked(crate = "luisa")]
pub fn linear_to_srgb1(rgb: Expr<f32>) -> Expr<f32> {
    rgb.le(0.0031308)
        .select(rgb * 12.92, rgb.powf(1.0f32 / 2.4) * 1.055 - 0.055)
}
#[tracked(crate = "luisa")]
pub fn rgb_to_hsv(rgb: Expr<Float3>) -> Expr<Float3> {
    let max = rgb.reduce_max();
    let min = rgb.reduce_min();
    let diff = max - min;
    let h = 0.0f32.var();
    if max == min {
        *h = 0.0;
    } else if max == rgb.x {
        *h = ((rgb.y - rgb.z) / diff).rem_euclid(6.0);
    } else if max == rgb.y {
...
#[tracked(crate = "luisa")]
pub fn hsv_to_rgb(hsv: Expr<Float3>) -> Expr<Float3> {
    let h = hsv.x;
    let s = hsv.y;
    let v = hsv.z;
    let f = |n: u32| {
        let k = (n as f32 + h * 6.0).rem_euclid(6.0);
        v - v * s * k.min_(4.0 - k).min_(1.0).max_(0.0)
    };
    let r = f(5);
    let g = f(3);
    let b = f(1);
