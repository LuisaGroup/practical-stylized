...
impl MeshAggregate {
    pub fn new(
        ...
        for (_i, mesh) in meshes.iter().enumerate() {
            aabb.0 = aabb.0.min(mesh.aabb.0);
            aabb.1 = aabb.1.max(mesh.aabb.1);
        ...
        let mesh_instances = device.create_buffer_from_fn(instances.len(), |i| {
            ...
            let mesh_aabb = meshes[inst.geom_id as usize].aabb;
            let extent = mesh_aabb.1 - mesh_aabb.0;
            for i in 0..8 {
                let x = (i & 1) as f32;
                let y = ((i >> 1) & 1) as f32;
                let z = ((i >> 2) & 1) as f32;
                let p = glam::Vec3::new(x, y, z);
                let p = mesh_aabb.0 + p * extent;
                let p = t.transform_point3(p);
                aabb.0 = aabb.0.min(p);
                aabb.1 = aabb.1.max(p);
            }
            MeshInstance {
