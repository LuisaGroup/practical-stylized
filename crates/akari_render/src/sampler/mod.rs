...
pub trait Sampler {
    ...
    fn start_at(&self, _sample_index: Expr<u32>) {
        unimplemented!()
    }
    fn sample_index(&self) -> Expr<u32> {
        unimplemented!()
    }
    fn start(&self);
...
impl Sampler for IndependentSampler {
    ...
    #[tracked(crate = "luisa")]
    fn start_at(&self, sample_index: Expr<u32>) {
        if let Some(_) = &self.states {
            self.state
                .advance((Self::MAX_DIM_PER_SPP as i64).expr() * (1 + sample_index.as_i64()));
        }
    }
    fn clone_box(&self) -> Box<dyn Sampler> {
        Box::new(Self {
...
impl Sampler for Pmj02BnSampler {
    ...
    #[tracked(crate = "luisa")]
    fn start_at(&self, sample_index: Expr<u32>) {
        *self.state.sample_index = sample_index;
    }
    #[tracked(crate = "luisa")]
    fn forget(&self) {
        *self.forget = true.expr();
