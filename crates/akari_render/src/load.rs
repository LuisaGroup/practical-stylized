...
// #[deny(dead_code)]
impl SceneLoader {
    pub fn load_from_path<P: AsRef<Path>>(device: Device, path: P, dry_run: bool) -> Arc<Scene> {
        ...
        let loader = Self::preload(device, Box::new(scene_view), dry_run);
        ...
        for (i, inst) in instances.iter_mut().enumerate() {
            ...
            let powers = {
                ...
                if !self.dry_run {
                    ...
                    let ones = vec![1.0f32; powers.len()];
                    powers.copy_from(&ones);
                }
    ...
    fn preload(device: Device, scene_view: Box<dyn SceneView + Sync>, dry_run: bool) -> Self {
                ...
                scenegraph::Geometry::Mesh(mesh) => unsafe {
                    macro_rules! load_slice {
                        ($s:expr, $t:ty) => {{
                            ...
                            // slice
                            let mut v: Vec<$t> =
                                Vec::with_capacity(slice.len() / std::mem::size_of::<$t>());
                            std::ptr::copy_nonoverlapping(
                                slice.as_ptr(),
                                v.as_mut_ptr() as *mut u8,
                                slice.len(),
                            );
                            v.set_len(slice.len() / std::mem::size_of::<$t>());
                            v
                        }};
                    }
                    ...
                    let mesh = Mesh::new(
                        ...
                        MeshRef::new(
                            &vertices,
                            normals.as_ref().map(Vec::as_slice),
                            &indices,
                            &materials,
                            uvs.as_ref().map(Vec::as_slice),
                            tangents.as_ref().map(Vec::as_slice),
                        ),
                    );
...
pub fn load_from_path<P: AsRef<Path>>(device: Device, path: P, dry_run: bool) -> Arc<Scene> {
    SceneLoader::load_from_path(device, path, dry_run)
}
