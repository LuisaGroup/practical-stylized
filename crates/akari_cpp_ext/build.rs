fn main() {
    ...
    println!("cargo:rustc-link-lib=dylib=akari_cpp_ext");
    bindgen::builder()
        .header("cpp_ext/akari_cpp_ext.h")
        .clang_args(&["-x", "c++", "-std=c++20", "-DBINDGEN"])
        .allowlist_file("cpp_ext/akari_.*")
        ...
        .unwrap();
    bindgen::builder()
        .header("cpp_ext/akari_nrc.h")
        .clang_args(&["-x", "c++", "-std=c++20", "-DBINDGEN"])
        .allowlist_file("cpp_ext/akari_.*")
        .enable_cxx_namespaces()
        .generate()
        .unwrap()
        .write_to_file("src/binding_nrc.rs")
        .unwrap();
}
